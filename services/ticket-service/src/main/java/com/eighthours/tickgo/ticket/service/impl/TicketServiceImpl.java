...
public class TicketServiceImpl implements TicketService {
    ...
    @Override
    public List<TrainOptionDTO> listAvailableTrains() {
        return trainMapper.selectList(
                        new LambdaQueryWrapper<TrainDO>()
                                .eq(TrainDO::getSaleStatus, 0)
                                .orderByAsc(TrainDO::getId))
                .stream()
                .map(train -> {
                    TrainOptionDTO dto = new TrainOptionDTO();
    ...
    @Override
    public List<String> listTrainStations(Long trainId) {
        return loadTrainStations(trainId).stream()
