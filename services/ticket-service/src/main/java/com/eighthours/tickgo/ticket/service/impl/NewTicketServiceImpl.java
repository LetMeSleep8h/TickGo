...
public class NewTicketServiceImpl implements NewTicketService {
    ...
    public SeatPreOccupyRespDTO purchaseTicketsV2(NewTicketPurchaseReqDTO request) {
        ...
        try {
            for (Long passengerId : request.getPassengers().stream()
                    .map(NewTicketPurchaseReqDTO.PassengerDTO::getPassengerId)
                    .sorted()
                    .toList()) {
                RLock passengerLock = redissonClient.getLock(buildPassengerLockKey(request.getTrainId(), passengerId));
                boolean locked = passengerLock.tryLock(3, 10, TimeUnit.SECONDS);
                if (!locked) {
                    throw new BizException("乘车人存在重复购票请求，请稍后重试");
                }
                lockList.add(passengerLock);
            }

            purchaseBusinessStateChecker.check(request);

            for (Integer seatType : seatTypeMap.keySet().stream().sorted().toList()) {
    ...
    private String buildPassengerLockKey(Long trainId, Long passengerId) {
        return "ticket:purchase:passenger:" + trainId + ":" + passengerId;
    }
        ...
        List<TicketDO> tickets = ticketMapper.selectList(
                new LambdaQueryWrapper<TicketDO>()
                        ...
                        .in(TicketDO::getTicketStatus,
                                TicketStatusEnum.WAIT_PAY.getCode(),
